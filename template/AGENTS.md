...
7. **Models:** UUID v7 PKs; `paranoid: true` soft-deletes (use `scope(null)` to bypass); always index FKs; carry the **owner FK onto every descendant** + protect with a composite FK; named indexes `{Table}_{col}_{idx|unique}` in BOTH model and migration.
